...
if(!args.igId || !args.token || !args.store || !args.out){
  console.error("usage: node fetch_reels.mjs --ig-id <ID> --token <TOKEN> --store <STORE> --out <DIR>");
  process.exit(2);
}

const API = (p, qs) => `https://graph.facebook.com/v23.0${p}?access_token=${encodeURIComponent(args.token)}${qs?`&${qs}`:""}`;
const MEDIA_FIELDS = ["id","media_type","permalink","timestamp","caption"].join(",");
const IS_REEL = (t)=> /REEL/i.test(t);
...
async function main(){
  ...
  for(let page=0; page<5 && url; page++){
    ...
    for(const it of (j.data||[])){
      if(IS_REEL(it.media_type)) list.push(it);
    }
  ...
  const items = list.map(x => ({
    ...
    metrics: {}  // v1では空。後段で拡張（plays, reach, avg_watch_time等）
  }));
